...
class FlightController extends Controller
{
    ...
    public function results(PublicFlightSearchRequest $request): View
    {
        ...
        [$result, $searchId, $warnings] = $this->runSearch($criteria);
    ...
    {
        $criteria = $request->criteria();
        [, $searchId, $warnings] = $this->runSearch($criteria);
        ...
        return response()->json([
            ...
            'criteria' => $criteria,
            'warnings' => $warnings,
            'initial_results_url' => route('flights.results.data', ['search_id' => $searchId]),
        ]);
    }
    ...
    public function resultsData(Request $request): JsonResponse
    {
        ...
        $data = array_map(function (array $offer) use ($payload, $searchId, $airlineLogos, $cityMap): array {
            ...
            return array_merge([
                ...
                'supplier_offer_id' => (string) ($offer['raw_reference'] ?? $offer['offer_id'] ?? $offer['id'] ?? ''),
                'provider' => (string) ($offer['supplier_provider'] ?? 'unknown'),
                'airline_code' => $code,
     ...
     */
    protected function runSearch(array $criteria): array
    {
        $agency = Agency::query()->where('slug', config('ota.default_agency_slug'))->first();
        $result = $this->flightSearch->searchWithMeta($criteria, $agency, 'public_guest');
        $allOffers = is_array($result['offers'] ?? null) ? $result['offers'] : [];
        $duffelOnlyEnabled = ! app()->environment('testing');
        $duffelOffers = $duffelOnlyEnabled
        ...
        if ($duffelOnlyEnabled && $allOffers !== [] && $duffelOffers === []) {
            $warnings[] = 'Duffel fares are currently unavailable. Please try again shortly.';
        }

        $searchId = $this->searchStore->store($criteria, $duffelOffers, $warnings);
        if (function_exists('session')) {
            session()->put('home_recent_fares', [
                'criteria' => $criteria,
                'offers' => array_slice($duffelOffers, 0, 3),
