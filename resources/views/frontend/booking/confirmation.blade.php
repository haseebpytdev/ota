    ...
    <section class="ota-confirmation-wrap ota-confirmation-page">
        <div class="ota-container ota-container-narrow">
            <div class="ota-confirm-hero-card">
                ...
                <div class="ota-confirm-meta">
                    <div class="ota-confirm-ref">
                        ...
                        @endunless
                        <span class="ota-confirm-ref__hint">{{ $paxCount }} passenger{{ $paxCount === 1 ? '' : 's' }}</span>
                    </div>
            ...
            <div class="ota-confirm-grid">
                ...
                <article class="ota-confirm-card">
                    ...
                    @foreach ($allPassengers as $idx => $pax)
                        <p class="ota-confirm-pax-name mb-1">
                            <strong>
                                Passenger {{ $idx + 1 }} — {{ ucfirst((string) $pax->passenger_type) }}
                                @if($pax->is_lead_passenger)
                                    <span class="label label-info">Lead</span>
                                @endif
                            </strong>
                            <span class="d-block">{{ trim(($pax->title ?? '').' '.($pax->first_name ?? '').' '.($pax->last_name ?? '')) ?: '—' }}</span>
                        </p>
                    @endforeach
            ...
            <nav class="ota-confirm-actions" aria-label="What would you like to do next?">
                ...
                <a href="{{ route('booking.lookup') }}" class="btn btn-primary btn-lg ota-confirm-btn-primary">Lookup booking</a>
