...
class AgentBookingController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if ($offer === null) {
            return back()->withErrors(['flight_id' => 'Selected mock flight is no longer available.'])->withInput();
        }
        ...
        $booking = DB::transaction(function () use ($agency, $agent, $validated, $criteria, $offer, $validation, $normalizedValidated): Booking {
            ...
            $booking->forceFill([
                ...
                'airline' => ($offer['airline_name'] ?? 'Mock Airline').' ('.($offer['airline_code'] ?? ($offer['carrier_code'] ?? 'XX')).')',
